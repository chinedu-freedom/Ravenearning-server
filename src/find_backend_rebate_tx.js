...
function searchBackend(dirPath) {
  ...
  for (const f of files) {
    ...
    if (stat.isDirectory()) {
      ...
      if (content.includes('Referral Commission') || content.includes('COMMISSION') || content.includes('rebate') || content.includes('Rebate')) {
        console.log(`Backend match in: ${fullPath}`);
      }
    }
  }
}
